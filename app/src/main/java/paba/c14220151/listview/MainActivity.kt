package paba.c14220151.listview
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //tambah data pada array yg ditampilkan ke ListView
        val _btnTambah = findViewById<Button>(R.id.btnTambah)
        _btnTambah.setOnClickListener {
            var dtAkhir : Int = Integer.parseInt(data.get(data.size-1))+1
            data.add(dtAkhir.toString())
            lvAdapter.notifyDataSetChanged()
        }

        //menampilkan data yang dipilih dari ListView
        _lv1.setOnItemClickListener { parent, view, position, id ->
            Toast.makeText(this, "ini data ke-${data[position]}",
